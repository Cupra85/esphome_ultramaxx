...
void UltraMaXXComponent::loop() {
    ...
    if (state == UM_RX) {
        ...
        if (!rx_buffer_.empty() &&
            (now - last_rx_byte_ > 400 ||
             (rx_buffer_.size() > 10 && rx_buffer_.back()==0x16))) {

            ESP_LOGI(TAG,"Parsing Frame (%d Bytes)...",(int)rx_buffer_.size());

            auto &f = rx_buffer_;

            // DATA beginnt bei Byte 7 laut Frame
            size_t ptr = 7;
            ...
            while (ptr + 2 < f.size()) {
                ...
                else if (total_energy_ && DIF==0x04 && VIF==0x06)
                    total_energy_->publish_state(decode_bcd(f,ptr,4)/1000.0f);

                else if (total_volume_ && DIF==0x0C && VIF==0x14)
                    total_volume_->publish_state(decode_bcd(f,ptr,4)/100.0f);

                else if (temp_flow_ && DIF==0x0A && VIF==0x5A)
                    temp_flow_->publish_state(decode_bcd(f,ptr,2)/10.0f);

                else if (temp_return_ && DIF==0x0A && VIF==0x5E)
                    temp_return_->publish_state(decode_bcd(f,ptr,2)/10.0f);

                else if (temp_diff_ && DIF==0x0B && VIF==0x61)
                    temp_diff_->publish_state(decode_bcd(f,ptr,3)/100.0f);
