...
void UltraMaXXComponent::loop() {
    ...
    if (state == UM_WAIT && now - state_ts_ > 500) {
    ...
    if (state == UM_SEND && now - state_ts_ > 500) {
        // Anfrage senden (REQ_UD2) - 0xFE ist die Broadcast Adresse
    ...
    if (state == UM_RX) {
        ...
        // Wenn wir Daten haben und seit 1,5s nichts mehr kam
        if (!rx_buffer_.empty() && (now - last_rx_byte_ > 1500)) {
            auto &f = rx_buffer_;
            ESP_LOGI(TAG, "Parsing %d Bytes...", f.size());
            
            // Hex-Dump für dich zum Prüfen im Log
            std::string hex; char b[4];
            for(uint8_t x : f) { sprintf(b, "%02X ", x); hex += b; }
            ESP_LOGD(TAG, "Daten: %s", hex.c_str());
        ...
        if (now - state_ts_ > 15000) {
            ESP_LOGW(TAG, "RX Timeout - Keine Antwort vom Zähler");
            rx_buffer_.clear();
            state = UM_IDLE;
        }
    }
