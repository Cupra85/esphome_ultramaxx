...
void UltraMaXXComponent::loop() {
  ...
  if (state==UM_RX) {
    ...
    // Suche gültigen MBus Frame wie pyMeterBus
    for(size_t i=0;i+6<rx_buffer_.size();i++){

      if(rx_buffer_[i]==0x68){
        ...
        if(L1==L2){
          ...
          size_t frame_len = L1 + 6;   // 68 LL LL 68 ... CS 16

          if(i+frame_len <= rx_buffer_.size()){

            if(rx_buffer_[i+frame_len-1]==0x16){

              ESP_LOGI(TAG,"Parsing Frame (%d Bytes)", frame_len);

              auto &f = rx_buffer_;

              for(size_t p=i; p+6<i+frame_len; p++){
