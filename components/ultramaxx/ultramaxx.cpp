...
void UltraMaXXComponent::loop() {
  ...
  if (state == UM_RX) {
    ...
    // ⭐ wichtig: UltraMaXX braucht lange
    if (!rx_buffer_.empty() && now - last_rx_byte_ > 1200) {

      ESP_LOGI(TAG,"Parsing Frame (%d bytes)", rx_buffer_.size());

      auto &f = rx_buffer_;

      // Frame muss mit 68 starten
      if (f.size() < 20 || f[0] != 0x68) {
      ...
      // DATA beginnt ab Byte 7 (laut Mikrocontroller.net)
      size_t i = 7;
