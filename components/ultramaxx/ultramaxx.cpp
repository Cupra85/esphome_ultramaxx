...
void UltraMaXXComponent::loop() {
    ...
    // --- SCHRITT 2: STATE MACHINE ---
    if (state == UM_WAKEUP) {
        if (now - last_send_ > 15) {
            uint8_t buf[20]; memset(buf, 0x55, 20);
            this->write_array(buf, 20); last_send_ = now;
        }
    ...
    if (state == UM_SEND && now - state_ts_ > 150) {
        ...
        rx_buffer_.clear(); // Puffer für sauberen Empfang nullen
        state = UM_RX; state_ts_ = now; last_rx_byte_ = now;
    }

    if (state == UM_RX) {
        // Trigger: Wir haben Daten und entweder 400ms Stille ODER Frame Ende erkannt
